...
export async function POST(req: NextRequest) {
  ...
  if (!vid || typeof vid !== "string" || !userId) {
    return NextResponse.json(
      { message: "Missing or invalid video ID or user ID" },
      { status: 400 }
    );
  }
  ...
  try {
    ...
    if (user.transcriptions_used >= 3) {
      return NextResponse.json(
        { message: "Free transcription limit exceeded" },
        { status: 403 }
      );
    }
    ...
    if (!video) {
      ...
      if (insertError) {
        ...
        return NextResponse.json(
          { message: "Error inserting new video" },
          { status: 500 }
        );
      }
...
async function generateSummary(transcript: string): Promise<string> {
  try {
    const response = await axios.post(
      "https://openrouter.ai/api/v1/chat/completions",
      {
        ...
        messages: [
          ...
          {
            role: "user",
            content: `Please summarize the following transcript in the main language of the video:\n\n${transcript}`,
          },
        ],
        max_tokens: 500,
      },
    ...
    return (
      response.data.choices[0].message.content || "Unable to generate summary."
    );
  } catch (error) {
